function generateunknown(n,p,q,nu,Treptition)
...
bGamma =tensor(zeros(1,p*q), [p q]);
bGamma(1,2)=2;
bGamma(2,3)=-2;
bGamma=sparse(double(bGamma));
listi=2:3;
listj=[1];
for i=listi
	for j=listj
...
csvwrite(append(path,'bGamma.csv'),full(bGamma));
...
for reptition=1:Treptition
    ...
    for i=1:n
        ...
        sigma = inv(omega);
        X(i,:) = mvnrnd(zeros(1,p),sigma,1)+Mu(i,:);
    end
